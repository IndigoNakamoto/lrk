...
const ROW_DEFS = [
  ...
  ["Reward", ext((x) => `${(x.reward / 1e8).toFixed(8)} LTC`)],
  ["Total Fees", ext((x) => `${(x.totalFees / 1e8).toFixed(8)} LTC`)],
  ["Median Fee Rate", ext((x) => `${formatFeeRate(x.medianFee)} lit/vB`)],
  ["Avg Fee Rate", ext((x) => `${formatFeeRate(x.avgFeeRate)} lit/vB`)],
  ["Avg Fee", ext((x) => `${x.avgFee.toLocaleString()} lit`)],
  ["Median Fee", ext((x) => `${x.medianFeeAmt.toLocaleString()} lit`)],
  ["Fee Range", ext((x) => x.feeRange.map(formatFeeRate).join(", ") + " lit/vB")],
  ["Fee Percentiles", ext((x) => x.feePercentiles.map((f) => f.toLocaleString()).join(", ") + " lit")],
  ...
  ["Total Input Amount", ext((x) => `${(x.totalInputAmt / 1e8).toFixed(8)} LTC`)],
  ["Total Output Amount", ext((x) => `${(x.totalOutputAmt / 1e8).toFixed(8)} LTC`)],
