...
export function createNetworkSection() {
  ...
  // Bound JS client may predate MWEB series; skip those leaves instead of crashing the tree.
  const mweb = outputs.mweb;
  // Share series are not in the generated by-type helpers yet; keep MWEB out of those compare trees.
  const hasMwebOutputTypes = Boolean(outputs.byType?.outputShare?.mwebPegPool);

  // Non-addressable script types, reverse creation with catch-alls at tail
  const nonAddressableTypes = [
    {
      key: "opReturn",
      name: "OP_RETURN",
      color: st.opReturn,
